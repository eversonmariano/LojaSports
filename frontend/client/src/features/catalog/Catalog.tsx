...
export default function Catalog(){
  ...
  if(!products) return <h3>Não é possível carregar produtos</h3>
  if(loading) return <Spinner message='Buscando seus Productos...'/>
  return (
    <Grid container spacing={4}>
      <Grid item xs={12}>
        <Box mb={2} textAlign="center">
          <Typography variant="subtitle1">
            Disponibilidade {(currentPage - 1) * pageSize + 1}-{Math.min(currentPage * pageSize, totalItems)} de {totalItems} itens
          </Typography>
        </Box>
      ...
      <Grid item xs={3}>
        ...
        <Paper sx={{ mb: 2, p: 2 }}>
          <FormControl>
            <FormLabel id="sort-by-name-label">Ordenar por Nome</FormLabel>
            <RadioGroup
        ...
        <Paper sx={{ mb: 2, p: 2 }}>
          <FormControl>
            <FormLabel id="brands-label">Marcas</FormLabel>
            <RadioGroup
              aria-label="brands"
              name="brands"
              value={selectedBrand}
        ...
        <Paper sx={{ mb: 2, p: 2 }}>
          <FormControl>
            <FormLabel id="types-label">Modalidade</FormLabel>
            <RadioGroup
              aria-label="types"
              name="types"
              value={selectedType}
